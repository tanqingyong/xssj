...
function cha_xun(){
	if(check_date()){
		$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/moneydetail/city_day.php');
	    $("form:first").submit();
	}
}
function GetParams() {
...
function export_excel(){
	if(check_date()){
		$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/moneydetail/export_city_day.php');
	    $("form:first").submit();
	}
}
</script>
...
<div id="coupons">
    <div class="dashboard" id="dashboard">
        <ul><?php echo mcurrent_moneydetail('city_day'); ?></ul>
    </div>
    <div id="content" class="signup-box">
        ...
        <table cellspacing="0" cellpadding="0" >
            <tbody>
            <tr><th width="">日期</th><th>大区</th><th>城市</th><th width="">销量</th><th width="">销售额</th><th>毛利</th><th>毛利率</th><th>累计销量</th><th>累计销售额</th><th>累计毛利</th><th>验证量</th><th>验证额</th><th>验证毛利</th><th>验证毛利率</th><th>累计验证量</th><th>累计验证额</th><th>累计验证毛利</th></tr>
            ...
            <tr><td><?php echo $data['data_date'];?></td>
                ...
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'goods_sum'];?></td>
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'sales_sum'];?></td>
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'profile_sum'];?></td>
                ...
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'verify_num_sum'];?></td>
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'verify_sum_sum'];?></td>
                <td><?php echo $sum_datas[$data['data_date'].$data['city'].'verify_profile_sum'];?></td>
            </tr>
            <?php }?>
            <?php if(!$datas){?>
