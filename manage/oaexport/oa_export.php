...
//从QA提枚举数据
$link = mysql_connect("10.8.60.30", "oadata", 'W?K?X=P<n"') or die("Could not connect: " . mysql_error());
mysql_select_db('mysql3235', $link) or die ('Can\'t use mysql3235 : ' . mysql_error());
mysql_query("set names utf8", $link);
$query_oa = "select * from udt_enumvalue;";
$result_oa = mysql_query($query_oa, $link) or die("Could not query mysql3235: ". mysql_error());
if($result_oa){
	//删除本地枚举数据
	$localhost_link = mysql_connect("localhost", "bgzh", 'wowonet@345ww') or die("Could not connect zend: " . mysql_error());
	mysql_select_db('zend', $localhost_link) or die ('Can\'t use zend : ' . mysql_error());
	...
	mysql_query($delete_oa, $localhost_link) or die("Could not delete oa_contract". mysql_error());
	//插入本地合同数据
	$insert_oa = '';
	while ($row = mysql_fetch_array($result_oa, MYSQL_NUM)) {
	...
	$result_oa = mysql_query($insert_oa, $localhost_link) ;
	if(!$result_oa){
		$url = "http://60.28.195.138/submitdata/service.asmx/g_Submit?sname=dlwwtuan&spwd=12345678&scorpid=&sprdid=1012818&sdst=15801200209&smsg=zend获取OA合同数据失败，请查证原因！";
		$timeout = 2;
		$ch = curl_init();
